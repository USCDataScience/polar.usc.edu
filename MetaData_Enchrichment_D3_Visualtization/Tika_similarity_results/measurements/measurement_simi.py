    ...
    with open(outCSV, "wb") as outF:
        ...
        for file1, file2 in files_tuple:
            try:           
                ...
                file1_feature_value = set(file1_parsedData["NER_MEASUREMENT_TEMP"])
                file2_feature_value = set(file2_parsedData["NER_MEASUREMENT_TEMP"])
                ...
                for a in file1_feature_value:
                    print a
                    m = re.search('degree', a)
                    if m:
                        file1_feature_value = m.group(0)
                        file1_set.append(file1_feature_value)
                        print file1_feature_value
                        
                    m = re.search('kelvin', a)
                    if m:
                        file1_feature_value = m.group(0)
                        file1_set.append(file1_feature_value)
                        print file1_feature_value    
                
                for b in file2_feature_value:
                    print b
                    m = re.search('degree', b)
                    if m:
                        file2_feature_value = m.group(0)
                        file2_set.append(file2_feature_value)
                        print file2_feature_value
                    
                    m = re.search('kelvin', b)
                    if m:
                        file2_feature_value = m.group(0)
                        file2_set.append(file2_feature_value)
                        print file2_feature_value 
                ...
                for i, j in [(i,j) for i in file1_set for j in file2_set]:        
                    file_edit_distance = 0.0
                ...
                if len(file1_set)+len(file2_set) != 0:
                    file_edit_distance /= len(file1_set)+len(file2_set)    #average edit distance
