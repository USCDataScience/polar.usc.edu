!function(){
	var bP={};	
	var b=10, bb=600, height=600, buffMargin=1, minHeight=14;
	var c1=[-130, 80], c2=[-50, 140], c3=[-10, 180]; //Column positions of labels.
	...
	function drawPart(data, id, p){
		...
		mainbar.append("text").attr("class","barlabel")
			.attr("x", c1[p]-20).attr("y",function(d){ return d.middle+5;})
			.text(function(d,i){ return data.keys[p][i];})
			.attr("text-anchor","start" );
			
		mainbar.append("text").attr("class","barvalue")
			.attr("x", c2[p]+30).attr("y",function(d){ return d.middle+5;})
			.text(function(d,i){ return d.value ;})
			.attr("text-anchor","end");
			
		mainbar.append("text").attr("class","barpercent")
			.attr("x", c3[p]+50).attr("y",function(d){ return d.middle+5;})
	...
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text("NER IN CCA Dataset")
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text("Value").attr("x", (c1[d]-5))
