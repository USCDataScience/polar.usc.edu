function refreshHzBarChart(){
    ...
    d3.json('json/NER_words.json', function(error, data) {
    ...
    bar.append("text")
        .attr("x", function(d) { if(x(d) < 6) return x(d) + 300; else if(x(d)<80) return x(d)+300; else if(x(d) < 160) return x(d)+ i +300; else if(x(d) < 300) return x(d)+ i + 300; else return x(d) + 300;})
        .attr("y", barHeight / 2)
        .attr("fill", function(d,i) { return colorbar[i % data.series.length]; })
        .attr("dy", ".35em")
