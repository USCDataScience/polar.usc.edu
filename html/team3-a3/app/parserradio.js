...
var maxmindata;
var globalKey;
$.get("maxminparser.txt", function(data, status) {
  data = JSON.parse(data);
  maxmindata = data;
  $.get("parserdata.json", function(ratiodata, status) {
  // console.log(data);
  resultdata = ratiodata;
    for(key in data) {
      ...
      createPlot('image/jpeg');
      $("#image\\/jpegradio").prop("checked", true);
  });
  // addLine(data, "blue");
});
...
function createPlot(key) {
    ...
    InitChart(xmax, ymax);
    var fingerprint = createXYDict(resultdata[key][1]);
    var correlation = createXYDict(resultdata[key][2]);
...
var tip = d3.tip()
  ...
  .offset([-10, 0])
  .html(function (d) {
    return resultdata[globalKey][0][d.x];
  });
var tipGuide;
var xRange;
var yRange;
...
function addLine(lineData, color, type) {
  ...
  .on('mouseover', function () {
    var m = d3.mouse(this);
    var x = Math.round(xRange.invert(m[0]));
    var d = lineData[x];
    tipGuide
      .attr('x', xRange(d.x))
      .attr('y', yRange(d.y));
