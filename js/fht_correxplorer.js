...
window.onload = function () {
    ...
    //        path = d3.select("input#file_path")[0][0].value;
    //        load_all(path);
    //    });
    //
    ...
    header_trailer = d3.select('input[name="header_trailer"]:checked').node().value;
    bytes = d3.select("select#bytes")[0][0].value;
    path = "../../data/mime-analysis-data/" + header_trailer + "/" + mime_type + "_" + header_trailer + "_profile_" + bytes + ".csv";
    console.log("PATH: " + path);
    //    d3.select("input#file_path")[0][0].value = path;
    load_all(path);
    bytes = d3.select("select#bytes")[0][0].value;

    var header_trailer = d3.select('input[name="header_trailer"]').node().value
    d3.select("input#trailer").on("change", function () {
        mime_type = QueryString.mime_type;
        header_trailer = this.value
        bytes = d3.select("select#bytes")[0][0].value;
        path = "../../data/mime-analysis-data/" + header_trailer + "/" + mime_type + "_" + header_trailer + "_profile_" + bytes + ".csv";
        console.log("PATH: " + path);
        //        d3.select("input#file_path")[0][0].value = path;
