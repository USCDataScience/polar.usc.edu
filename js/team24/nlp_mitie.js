function refreshHzBarChart(){
    ...
    d3.json('http://skashyap7.github.io/TREC-Polar-Evaluation/data/team24/words.json', function(error, data) {
    ...
    bar.append('rect')
        .attr("x", function(d) {
         if(x(d) < 6) return x(d) + 80; else if(x(d) < 80) return x(d) + 120; else if(x(d) < 300) return x(d) + 5 ; else return x(d) - 65; })
        .attr("y", 1)
        .attr('width', 65)
    ...
    bar.append("text")
        .attr("x", function(d) { if(x(d) < 6) return x(d) + 160; else if(x(d)<80) return x(d)+80; else if(x(d) < 160) return x(d)+ i +100; else if(x(d) < 300) return x(d)+ i + 120; else return x(d) + 80;})
        .attr("y", barHeight / 2)
        .attr("fill", function(d,i) { return colorbar[i % data.series.length]; })
        .attr("dy", ".35em")
